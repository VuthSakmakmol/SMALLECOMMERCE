...
async function refreshSavedOrder(){
  ...
  }catch{ localStorage.removeItem(ORDER_KEY); currentOrder.value = null }
}
...
<template>
  <div>
    ...
    <v-alert v-else :type="currentOrder.status==='READY' ? 'success':'info'" variant="tonal">
      <div><b>#{{ currentOrder._id?.slice(-6) }}</b> — {{ statusText[currentOrder.status] || currentOrder.status }}</div>
      <div class="mt-1 text-medium-emphasis">
        Items: {{ (currentOrder.items || []).map(i => (i.qty||1)+'× '+i.nameSnapshot).join(', ') }}
      </div>
      <div class="mt-2" v-if="currentOrder.status==='READY'">
        <v-btn size="small" color="success" @click="markDelivered">I got it</v-btn>
      </div>
    </v-alert>
  </div>
</template>
